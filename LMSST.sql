...
--
-- Struktura widoku `stck_vstockcount`
--

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `stck_vstockcount` AS (select `s`.`productid` AS `productid`,count(`s`.`id`) AS `scount` from `stck_stock` `s` where isnull(`s`.`pricesell`) group by `s`.`productid`);
